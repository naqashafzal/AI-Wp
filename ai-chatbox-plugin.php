...
require_once AICB_PLUGIN_DIR . 'inc/download-button.php';
...
/**
 * Remove the WordPress version from the footer on plugin pages.
 */
add_filter('update_footer', 'aicb_remove_footer_version', 999);
function aicb_remove_footer_version($text) {
    $current_screen = get_current_screen();
    if ($current_screen && strpos($current_screen->id, 'aicb-') !== false) {
        return '';
    }
